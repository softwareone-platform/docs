...
When a subscription has returnable orders, you can only downsize by quantities that match the sum of orders within the return window.&#x20;
...
Additionally, the renewal quantity of the subscription is reduced to match the new quantity of licenses after the return has been processed.

In cases where your requested reduction quantity doesn't match the quantity of one or more returnable orders, the system prevents the reduction from being processed and suggests a valid quantity that can be returned.

### Downsizing a subscription without returnable orders

When a subscription doesn't have any returnable orders, only the renewal quantity of the subscription is reduced. This ensures that at the next renewal, only the reduced quantity of licenses is renewed.
...
To disable or enable autorenewal for a subscription, use the configuration wizard in the SoftwareOne Marketplace to place a configuration order for an existing subscription.
...
You can terminate a subscription if the entire quantity of its licenses is still within the return window.&#x20;

# Rename Adobe Subscription

Your account on the Marketplace Platform may have several Adobe subscriptions. You can rename these subscriptions so you can identify them easily.&#x20;

In this tutorial, you'll learn how to rename a subscription from _Creative Cloud All Apps Pro for Enterprise_ to _Creative Cloud test._

## Prerequisites

Before starting this tutorial, make sure that the subscription you want to rename is in the **Active** or **Updating** state. Terminated subscriptions cannot be renamed.

## Implementation

{% stepper %}
{% step %}
**Open the subscription to rename**

Navigate to the **Subscriptions** page. Then, select the subscription you want to rename.

On the details page, select the arrow <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#5f6368&#x22;><path d=&#x22;M480-344 240-584l56-56 184 184 184-184 56 56-240 240Z&#x22;/></svg>" data-size="line">and choose **Rename**.&#x20;
{% endstep %}

{% step %}
**Provide the new subscription name**

In the **Rename** dialog, enter the new name in the **Subscription name** field. Then, select **Save**. In this example, the subscription's name is changed to **Creative Cloud test**:

<figure><img src="../../../.gitbook/assets/image (1065).png" alt=""><figcaption><p>Subscription name</p></figcaption></figure>

The new name is updated immediately and reflected on all pages that feature the subscription in the platform, including the subscription details page.&#x20;
{% endstep %}
{% endstepper %}

...
2. Select **Migrate EA API**.&#x20;
3. In the **Migrate to EA API** window, enter the enrollment number and select **Migrate**.&#x20;
...
5. On the consent page, review the permissions required by the Client Portal and select **Accept** to grant consent.
...
2. In the **Actions** column, select **Migrate EA API**.&#x20;
3. In the **Migrate to EA API** window, enter the enrollment number, then select **Migrate without consent**.&#x20;

<figure><img src="../../../../.gitbook/assets/Migrate.png" alt="" width="509"><figcaption><p>Migrate without consent </p></figcaption></figure>

4. On the details page of the tenant, select the **Enrollment Numbers** tab and select **Show manual steps**.
5. In **Manual steps**, assign permissions using Cloud Shell or REST API and select **Close**. Note that you must have the **Azure EA Enterprise Administrator** role to assign permissions.

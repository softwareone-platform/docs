---
description: Download your billing data.
---

# Export billing data

The **Export** option on the **Analytics** page allows you to download your billing data as an Excel file, so you can share it with others by email or other methods.

While the visual chart within **Analytics** displays only your top 10 grouped entities, the Excel file provides you with a complete and unfiltered dataset.

### Exporting billing data

To export billing data:

1. Go to **Billing** > **Analytics**.
2. In the filters sidebar, apply your desired filters, then select **Show results**.
3. Under **Analytics dashboard**, select **Export** to download the file.

<div data-with-frame="true"><figure><img src="../../../.gitbook/assets/Analytics.png" alt=""><figcaption><p>Use the Export option to download your billing data.</p></figcaption></figure></div>

...
4. In the **Terminate agreement** wizard, do the following:
   1. On the **Items** page, review the quantity details. All items will have the new quantity as **0**. Then, select **Next**.
   2. On the **Order details** page, add or update the additional ID and notes as needed. Select **Next**.&#x20;
   3. On the **Review order** page, select **Place order** to place the termination order. Then, click **View order** to open the details page of the termination order. Otherwise, click **Close**.&#x20;

Once the order has been placed, the agreement status changes from **Active** to **Updating**. After the termination order has been processed and all subscriptions within the agreement are canceled, the status changes to **Terminated**.

---
description: Complete (or close) a case chat if your issue is resolved.
---

# Complete case

You can mark a case as **Complete** when your issue has been resolved and no further assistance is needed.&#x20;

Completing a case closes it. Any user in the account can close a case, regardless of who created it.&#x20;

If additional assistance is required later, you can reopen the case. For more information, see [Reopen case](reopen-case.md).

### Complete a case from the Cases page

To close a case from the **Cases** page:

1. Go to **Helpdesk** > **Cases**.
2. (Optional) Use filters to find the desired case.
3. Select the case ID.&#x20;
4. In the details pane, select the actions icon (**•••**).
5. Under **Case**, select **Complete**.
6. Confirm that you want to complete the case. The case status changes to **Completed**.

### Complete a case from the Chats page

To close a case from the **Chats** page:

1. Go to **Chats**.&#x20;
2. (Optional) Use <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#5f6368&#x22;><path d=&#x22;M400-240v-80h160v80H400ZM240-440v-80h480v80H240ZM120-640v-80h720v80H120Z&#x22;/></svg>" data-size="line">**Filters** at the top of your chat list to find the desired case.
3. Select the case.&#x20;
4. In the chat window, select the actions icon (**•••**).
5. Under **Case**, select **Complete**.
6. Confirm that you want to complete the case. The case status changes to **Completed**.

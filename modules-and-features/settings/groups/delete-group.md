---
description: Delete a user group permanently.
---

# Delete groups

If a group is no longer needed, you can delete it. All groups except the default group can be deleted. Once a group is deleted, it cannot be restored.

{% hint style="info" %}
Before deleting a group, make sure to remove all members from the group. If the group contains members, the platform displays a message.&#x20;
{% endhint %}

### Delete a group

To delete a group permanently:

1. Go to **Settings** > **Groups**.&#x20;
2. Find the group you want to delete.&#x20;
3. Do one of the following:
   * Select the actions icon (**•••**) for the group and select **Delete**.&#x20;
   * Select the group to open its details page. Then, select the dropdown arrow <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#5f6368&#x22;><path d=&#x22;M480-344 240-584l56-56 184 184 184-184 56 56-240 240Z&#x22;/></svg>" data-size="line">, and choose **Delete**.&#x20;
4. In the **Delete group** dialog, select **Delete** to confirm. A confirmation message is displayed stating that the group has been deleted.&#x20;

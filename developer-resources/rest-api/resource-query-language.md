...
2. **Filter by Parent ID**: Use a direct filter parameter if available, or RQL.
   * _Finding_: The API often accepts direct property filters like `?agreement.id=AGR-XXX`.
3. **Minimize Payload**: Set `limit=0`.
   * _Finding_: `limit=0` is fully supported and recommended. It returns an empty `data` array but includes the correct count in `$meta.pagination.total`.
...
_Note: Using `limit=0` is the most efficient way to get counts as it minimizes data transfer._
...
> \[!IMPORTANT] **API Date Requirement**: Dates must be in **UTC** and include **3-digit millisecond precision** (e.g., `YYYY-MM-DDTHH:mm:ss.sssZ`).
...
* **Buyer**: Represents a legal entity in the platform.
  ...
  * _Note_: This is the core entity for billing and legal purposes.

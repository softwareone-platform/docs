# How do I sort and filter data?

## Filter data

Filters help you to narrow down data based on specific attributes. In the Marketplace Platform, you can use various filters to display only the information you need and hide all other data from the page.&#x20;

Depending on the type of data you are searching for, you can use a single filter or create multiple filters using different conditions and AND/OR operators. For instance, when ordering new items, you could create a filter to show only the items with a specific name. This is especially helpful for products containing several items, allowing you to find them quickly.&#x20;

Similarly, if you have multiple subscriptions, you could use the AND/OR operators to view subscriptions that are active AND have auto-renewal enabled. The following steps describe how to use filters:&#x20;

1. Navigate to the page containing the data you want to filter.&#x20;
2. Click the <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#5f6368&#x22;><path d=&#x22;M400-240v-80h160v80H400ZM240-440v-80h480v80H240ZM120-640v-80h720v80H120Z&#x22;/></svg>" data-size="line"> **Filter** button (located in the toolbar above the column header).
3. In the **Filters** box, click **Add another condition**.
4. Define the conditions:
   1. Choose the required property.&#x20;
   2. Select the condition, such as, equal, contains, starts with, and so on.&#x20;
   3. Choose a value from the list or type the keyword.&#x20;
5. If needed, add a new condition and repeat step 4. You can then combine conditions using the **AND** or **OR** operators.&#x20;
   * AND – If you select this operator, the results are displayed only if both conditions are met.
   * OR – If you select this operator, only one of the conditions needs to be met for the results to be displayed.

As you define filters, the data is refreshed automatically. If no data matches your filters, the platform displays a message. &#x20;

The following example shows how you can use filters to quickly find an item using the item name:

<figure><img src="../../.gitbook/assets/image (2).png" alt=""><figcaption></figcaption></figure>

## Sort data

Sorting allows you to reorganize your data in ascending or descending order so you can understand and visualize it better. There are two ways to sort data in the platform.&#x20;

To sort the data for a particular column, click the column header.&#x20;

To sort the entire table, click the <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#5f6368&#x22;><path d=&#x22;M320-440v-287L217-624l-57-56 200-200 200 200-57 56-103-103v287h-80ZM600-80 400-280l57-56 103 103v-287h80v287l103-103 57 56L600-80Z&#x22;/></svg>" data-size="line">**Sort** button above the column header and follow these steps:

1. In the **Sort** box, click **Add another sort** and then choose the required property.&#x20;
2. Choose the ascending or descending order for the display.

The following example shows how you can sort your subscriptions based on their status:

<figure><img src="../../.gitbook/assets/Sort.png" alt=""><figcaption><p>Sort option</p></figcaption></figure>

## Show or hide columns

The <img src="data:image/png;base64," alt="<svg xmlns=&#x22;http://www.w3.org/2000/svg&#x22; height=&#x22;24px&#x22; viewBox=&#x22;0 -960 960 960&#x22; width=&#x22;24px&#x22; fill=&#x22;#5f6368&#x22;><path d=&#x22;M121-280v-400q0-33 23.5-56.5T201-760h559q33 0 56.5 23.5T840-680v400q0 33-23.5 56.5T760-200H201q-33 0-56.5-23.5T121-280Zm79 0h133v-400H200v400Zm213 0h133v-400H413v400Zm213 0h133v-400H626v400Z&#x22;/></svg>" data-size="line"> **Columns** selector allows you to show or hide specific columns, providing you with the flexibility to display only the relevant information.

<figure><img src="../../.gitbook/assets/image (482).png" alt=""><figcaption><p>Columns selector</p></figcaption></figure>

* To hide a column, clear the checkbox for that column.
* To show a column, select the check box for that column.

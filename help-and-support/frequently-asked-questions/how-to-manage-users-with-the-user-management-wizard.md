...
The new version of User Management is currently available for most customers. If you cannot see the User Management wizard, then SoftwareONE will need to migrate your data first.

### Introduction <a href="#post-4972-_ref61592726" id="post-4972-_ref61592726"></a>

The User Management feature allows Administrators to do the following:
...
* Disable users to block their access&#x20;
* Delete users

\*… this functionality only applies to certain features.

### Access User Management <a href="#post-4972-_toc62223014" id="post-4972-_toc62223014"></a>

The User Management feature can only be accessed by users that have admin privileges. Admin privileges are set up for your specified users during the onboarding process.
...
The User Management main page shows a complete list of users. The following user information is displayed:
...
User Management has a wizard that guides you through the process of adding a user. The same structure is also used to edit an existing user.

The steps are shown in 3 different visual ways depending on their status:

* **Green number** – these are steps that can be done.
* **Gray** – these are steps that can only be done once another step has been completed. The step number and name of the step are grayed out.
* **Green tick** – steps that are done are ticked off in green. That means that a User Management administrator has explicitly saved this step at least once.
...
Continue to manually add the user by specifying the account security settings if they are required:

* **Multi-Factor-Authentication (MFA) after inactivity:** If the user has not logged on for a specified number of days, they are required to use a second factor before they can login to again.
* **Multi-Factor-Authentication (MFA) for every login:** Users are required to use a second factor for every login. This setting is enabled by default.
* **Automated account blocking:** If the user has not logged on for a specified number of days, then they are automatically blocked and cannot log in before being unblocked.
* **Password expiration:** Number of days after which the user must change their password. This setting can only be specified for users who log in with a username and password.

### Feature Permissions <a href="#post-4972-_ref61616565" id="post-4972-_ref61616565"></a>

Specify which features the user is allowed to use in this step:

* **Select the user type**
  * Administrators have access to User Management and can manage users and their access
...
* **Enable feature permissions for a user**
  ...
  * Use the toggle to grant or revoke permissions. If used on the feature level, it will grant or revoke all permissions of the feature
  * The toggle will only apply to the search results

The “Procurement (excluding Office365)” role is visible to all customers and can be used by all Administrators. Other roles will only be visible if customers have purchased the corresponding service from SoftwareONE.

Click on the role again to disable the role and its predefined permissions. Click on enabled permissions to disable them. This might add the text “edited” at the end of the role to indicate that only some permissions of the role are enabled.

* If an “i” icon is displayed next to permission, hover over the icon to read a more detailed description.

**Note:** User Management Administrators can only give users access to features that their own user has. This is the reason why User Management Administrators cannot manage the feature permissions of their own user. If they removed feature permissions from their user, then they would not be able to give access to those permissions to other users anymore.

### Company Access <a href="#post-4972-_toc62223022" id="post-4972-_toc62223022"></a>

Manage the companies to which users get access in this step. The “Company Access” step is only available when permission is enabled for a user that requires them to define the company access, e.g. procurement.
...
User Management provides extensive capabilities to define user data access for Spend Management. It is possible to limit user access by Accounts or Custom Groups. This step is only available when permissions for a “Spend Management” feature is enabled for the user.
...
**Note:** The user will be able to see all resources assigned to the assigned groups, and all Spend Management data related to those resources.
...
In this step, you can manage the user access to folders that exist in the Collaboration Site. It is only available when a “Collaboration Site” permission is enabled for the user.
...
This is the last step before informing the new users about their access or the starting point when viewing existing users.
...
You are now ready to inform new users about their access.

The welcome e-mail can only be sent once and the step will no longer be shown in the User Management wizard.

If users delete or lose their welcome e-mail, they can go to the login page where they can click on the “Don’t remember your password?” link to reset their password

**Note:** If User Management administrators want to hand over credentials to their users later (e.g. after an introduction meeting), then they can either wait with the “Inform User” step or send the welcome e-mail to themselves and forward it separately.
